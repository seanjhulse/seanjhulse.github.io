...
class Sockets extends Component {
	...
	render() {
		...
		return (
			<article>
				<h1>Sockets</h1>
				<p>I played around with TCP and some UDP sockets in Python. Turns out sockets aren't that hard to setup (with help from <a href="https://realpython.com/python-sockets/">https://realpython.com/python-sockets/</a>), but they're pretty hard to manage. It's easy to end up breaking your server. Even my best multi-connection servers would break down if I sent ten thousand small requests in a loop. Using the selectors package, we can handle connections simultaneously as quickly as possible, but our server was a little fragile. If things went wrong, it would crash or stall. Sometimes it could churn through them, but at great cost to my little processor.</p>
				<p>I'm not going to cover the full-tutorial from realpython. They covered it very well and it's lengthy indeed. Instead, I recommend you give it a read.</p>
				<p>Since they did such a good job covering it, I'm just going to describe some terms in some fun ways to flex my animation muscle a little.</p>
				<div className="terms">
					<div className="term">
						<div className="term-name" style={styles["term-name"]}>UDP</div>
						<button onClick={this.udp}>Serve data over UDP</button>
						<button onClick={this.organize_udp} disabled={udp_client_data.length < this.state.udp_server_data.length}>Sort UDP Bits</button>
						<p className="term-animation">
							{udp_client_data.map(value => value + " " )}
						</p>
					</div>
					<div className="term">
						<div className="term-name" style={styles["term-name"]}>TCP</div>
